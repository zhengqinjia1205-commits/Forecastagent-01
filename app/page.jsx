...
export default function Page() {
  ...
  async function onSubmit(e) {
    ...
    form.append("test_size", String(toNumber(testSize, 0.2)))
    form.append("random_seed", String(toNumber(randomSeed, 42)))
    form.append("periods", String(toNumber(periods, 10)))
    form.append("api_base", normalizeBaseUrl(apiBase))
    if (String(timeCol || "").trim()) form.append("time_col", String(timeCol).trim())
    if (String(demandCol || "").trim()) form.append("demand_col", String(demandCol).trim())
    const methods = []
    ...
    if (methods.length) form.append("methods", methods.join(","))
    ...
    try {
      ...
      setError(err?.message || "请求失败（可能是后端未启动或API地址不可达）")
    } finally {
      setLoading(false)
    }
  }
